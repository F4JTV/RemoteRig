...
C++17 / Qt6 / PortAudio / Opus / Hamlib. Same code on Windows, Linux and Android.

*Version française : [README.fr.md](README.fr.md)*


<p align="center">
  <img src="docs/images/RemoteRig_Client_Android.png" alt="Touch client" width="260">
</p>

<p align="center">
  <img src="docs/images/RemoteRig_Client_Station.png" alt="Desktop client" width="680">
</p>

<p align="center">
  <img src="docs/images/RemoteRig_Server_Radio.png" alt="Station server" width="560">
</p>

*The touch client on Android, the desktop client connected to an FT-891, and the
station server. The other tabs are covered in the user manual, under
Help → Manual.*
...
```
  ...
  │ rigctld :4532    │                        │                    │
  │  ↑ WSJT-X, fldigi│◀─── TCP control ──────▶│ Hamlib / RTS / DTR │──▶ CAT + PTT
  └──────────────────┘   (framed JSON)        └────────────────────┘
```
...
Two routes, depending on what the rig accepts.
...
generated by the server* below. The network never takes part in the spacing:
only the text travels.

The rig's own keyer appears only when it supports Morse. Hamlib has no
capability flag for it, so the server checks whether the backend provides a
`send_morse` function at all; on a rig without one, nothing is shown rather than
a control that fails silently.

On the phone, a dot-and-dash button sits in the top bar and opens a panel from
the top edge: speed, six software memories, the rig's own five, a free-text
field, and a stop button kept within reach — a memory fired by mistake has to be
stoppable without hunting for the control. On the desktop the same functions
live in a **CW** tab.

Software memories use `%c` for the operator's callsign, so they stay valid
whoever uses the application. Speed goes through `RIG_LEVEL_KEYSPD`, and also
paces the server's own keyer.
...
### CW generated by the server

Some rigs will not accept free text over CAT: the command is rejected, or the
rig keys one of its own stored memories instead of the text. For these, the
server can produce the elements itself and key a serial line wired to the rig's
KEY jack.

Try the rig's own keyer first — it needs no wiring, and on an FT-891 over CAT it
does send the typed text. This is the fallback for when it does not.

Tick **Generate CW here and key a serial line**, choose the port and whether
DTR or RTS carries the keying. The line can be inverted for interfaces that
present it the other way round, and the PTT can be held for the whole message
on rigs that do not key themselves.

**Correction** shortens every keyed element, never the silences, to make up for
the time the rig takes to raise its carrier. Start at zero and raise it only if
your correspondents report clipped dots.

The network never takes part in the spacing: only the text travels, and the
rhythm is produced next to the radio. The keyer runs in its own thread — rig
polling does serial reads that can last tens of milliseconds and would
otherwise shift the elements — and each element's deadline is counted from a
single reference instant, so a late timer does not carry over to the rest.
Measured on a virtual port: "PARIS PARIS" at 20 WPM took 5581 ms against 5580
expected.

The keying port must be one the rig control is not already holding: a serial
port cannot be opened twice. Rigs that expose two USB serial ports, and
interfaces such as the SCU-17, give you the second one for this.


### Write then read back, and raw CAT

Every frequency, mode and VFO change is **read back** after being written. Rigs
round to their VFO step, refuse silently outside their ranges, or fall back to a
neighbouring mode; without the read-back the operator would believe the command
took. When what the rig reports differs, the log says both: what was asked and
what came back.

A **raw CAT** field sends a sequence to the rig untouched, for the settings no
driver covers — a menu peculiar to one model, a rare function. The terminator is
added when missing, nothing else is interpreted, and the rig's answer comes back
in the log. It has its own **CAT** tab in the desktop client, with a short guide to the
...
PTT, S-meter, SWR. Without care, a band button pressed mid-cycle waits for all
six before its command leaves. On a rig with a slow CAT timeout that is felt.

Every operator command therefore drops a token the moment it is emitted, from
whichever thread emitted it, and the polling checks that token between reads. It
gives up the rest of its cycle and lets the command through. The state stays
consistent because the cycle starts from a copy of the current state: a field
that was not re-read simply keeps its value.

A safety net bounds the effect. If a token were ever left in the air — a command
...
```bash
./make_deb.sh            # asks before installing what is missing
./make_deb.sh --deps     # installs without asking
./make_deb.sh --no-deps  # checks nothing
```

It also caps the number of parallel compile jobs by available memory rather than
by core count alone: compiling Qt code takes around 700 MiB per job, and a 2 GiB
Raspberry Pi 4 running four of them gets killed by the kernel partway through.
On a 2 GiB Pi it runs two, on a 1 GiB Pi one.
...
```bash
./install.sh            # asks before installing what is missing
./install.sh --deps     # installs without asking
./install.sh --no-deps  # checks nothing
```

They also cap the number of parallel compile jobs by available memory rather
than by core count alone.
...
with Qt 6.4.2, Hamlib 4.5.5, PortAudio 19 and Opus. All three executables start and hold their event loop under
...
10 kHz tone decimated to 16 kHz comes out at -53 dB. All 416 translatable
strings are translated and checked at runtime.

The protocol is exercised by two probes that speak it by hand, without going
through the client: wrong passwords, forged MACs, malformed frames, absurd frame
lengths, forged UDP PTT packets, a second client, and a client that vanishes
silently while transmitting. See `test/`.

All three systems have been built and run against real hardware: a Yaesu FT-891
over CAT, an FT-8800 on serial PTT alone, with the audio judged on the air by
correspondents and the latency measured over 4G from a phone.

A few notes kept from that work:

- The Windows build needs Visual Studio Build Tools 2022 (MSVC 14.44) with
  Qt 6.11.2. Two problems came up and are fixed: `M_PI`, which MSVC does not
  define unless `_USE_MATH_DEFINES` comes first, and the missing `<pthread.h>`
  pulled in by the Hamlib headers.
- Resources are compressed with **zlib**, not the default zstd. `rcc` otherwise
  guards on a symbol that QtCore only exports when built with zstd, and a binary
  linked against a Qt that has it refuses to start against one that does not.
- On Linux, the serial device path handed to Hamlib is the **system path**,
  `/dev/ttyUSB0`, not the bare name Qt reports: Hamlib treats anything that does
  not look like a device file as a network address.
...
The client runs the same C++ on Android, with a Qt Quick interface made for a
finger. Audio, PTT, CAT, keyer and themes are all in place.
