...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/platforms-Windows%20%7C%20Linux%20%7C%20Android-lightgrey.svg" alt="Platforms">
</p>
...
- **Two-way audio** at 48 kHz, Opus for voice or 16-bit PCM for data modes, with
  an adaptive jitter buffer and a measured round trip of a few milliseconds on a
  local network.
- **PTT** by CAT command, RTS, DTR, a CM108 sound chip's GPIO3 line, or a tone
  on the right audio channel.
- **Full CAT control** through Hamlib: frequency, bands, modes, filter widths,
  VFO, antenna tuner, S-meter and SWR — whatever the rig reports, never what was
  asked for.
- **CW** from the rig's own keyer, from its stored memories, or generated by the
  server and keyed on a serial line for rigs that refuse free text.
- **Raw CAT** commands with saved macros, for what no driver covers.
- **Data modes** through a virtual audio cable and a local rigctld interface, so
  WSJT-X, fldigi or JS8Call drive the remote rig as if it were on the desk.
- **Three clients from one codebase**: a desktop client, a touch client for
  Android, and a headless server for a Raspberry Pi.
- **Safety**: band-edge guard on the transmitted spectrum, a watchdog that stops
  transmission two seconds after a client falls silent, challenge-response
  authentication and optional encryption.

## Quick start

```bash
./build_hamlib.sh     # Hamlib 4.6 or later, if your distribution ships an older one
./install.sh          # build and install the server and the desktop client
remoterig-server      # on the machine next to the radio
remoterig-client      # wherever you are
```

The server's Network tab shows the address to point the client at. Everything
else is in the user manual, under Help → Manual.
...
- [Choosing the codec](#choosing-the-codec)
- [Microphone shaping](#microphone-shaping)
- [Running the server without a desktop](#running-the-server-without-a-desktop)
- [Automatic reconnection](#automatic-reconnection)
- [Sending CW](#sending-cw)
- [SWR](#swr)
- [Band-edge guard and level meters](#band-edge-guard-and-level-meters)
- [Modes and filter width](#modes-and-filter-width)
- [PTT methods](#ptt-methods)
- [Bands and antenna tuner](#bands-and-antenna-tuner)
- [User manual](#user-manual)
- [Client watchdog](#client-watchdog)
- [Security](#security)
- [Radios without CAT](#radios-without-cat)
- [Data modes](#data-modes)
- [Raspberry Pi 4 and 5](#raspberry-pi-4-and-5)
- [Ports to open](#ports-to-open)
- [Level setting](#level-setting)
- [Source layout](#source-layout)
- [State of the code](#state-of-the-code)
- [What is in place](#what-is-in-place)
- [The touch interface](#the-touch-interface)
- [Staying alive with the screen off](#staying-alive-with-the-screen-off)
- [Themes](#themes)
- [Audio device, PTT key and rigctld](#audio-device-ptt-key-and-rigctld)
- [What is still missing](#what-is-still-missing)
- [Versioning](#versioning)
- [Building the APK on Ubuntu 24.04](#building-the-apk-on-ubuntu-2404)
