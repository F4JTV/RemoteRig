...
It does not work everywhere. On Yaesu HF sets, the keyer command only replays
the rig's internal memories: sending `AGN?` plays memory 1 whatever the text.
Five buttons therefore call those memories directly, numbered 1 to 5, in the CW
panel and in the desktop client's tab.
...
Some rigs will not accept free text over CAT: their keyer command only replays
their own memories. Yaesu HF sets behave this way — sending `AGN?` plays
memory 1, whatever the text. For these, the server can produce the elements
itself and key a serial line wired to the rig's KEY jack.
